...
def oci_bundle_bundle_conf(args, component_name):
    ...
    oci_tree.put('endpoints', ConfigTree())
...
def oci_bundle_unpack(destination, data, is_dir):
    temp_dir = tempfile.mkdtemp()

    try:
        if is_dir:
            shutil.copytree(data, os.path.join(temp_dir, 'bundle'), symlinks=True)
        else:
            data.extractall(temp_dir)

        for base, dirs, files in os.walk(temp_dir):
            if 'config.json' in files or 'refs' in dirs:
